(define (domain ejercicio_5)
	...
	(:types
        ...
        investigacion - elementos_mapa ; Diferenciamos que un elemento del mapa puede ser una investigacion
		...
		nombre_investigacion - investigacion 		; Tipo concreto para las constantes de investigacion
    ) 
	...
	(:constants
        ...
        BahiaDeIngenieria - nombre_edificio						; Constantes de edificios para todo el problema 

        Mineral Gas - nombre_recursos_entorno					; Constantes de recursos para todo el problema 

        ImpulsarSegador - nombre_investigacion					; Constantes de investigacion para todo el problema 
	...
	(:predicates
        ...
        (requiere_recurso_tipo ?nombre_edfcio - elementos_mapa ?rec - nombre_recursos_entorno)

        ; guarda a que tipo pertenece una investigacion concreta (Por ejemplo, ImpulsarSegador1 es de tipo ImpulsarSegador)
		(investigacion_de_tipo ?investgacion - investigacion ?nombre_investgacion - nombre_investigacion)

		; indica que una investigacion aun no se ha realizado, condicion necesaria para reclutar Segadores
		(por_investigar ?investgacion - investigacion) 
    ...
    (:action reclutar
        ...
        :precondition
            (and
                ...
                ; tenemos que tener algun vce extrayendo cada recurso que necesite una unidad antes de reclutarla 
                (and 
                	(exists (?t - nombre_unidad)  ;existe al menos un tipo de unidad
                    	(and
                     
                        	(unidad_de_tipo ?unidd ?t) ;que sea el tipo de unidad que vamos a reclutar
                        
                        	;si ese tipo de unidad necesita Mineral tenemos que tener algun vce extrayendolo ya
                        	(imply (requiere_recurso_tipo ?t Mineral) (recolectando_recurso_entorno Mineral) )
                    	)
	                )
                    ...
                    (exists (?t - nombre_unidad)  ;existe al menos un tipo de unidad
                    	(and
                        	...
                        	;si ese tipo de unidad necesita Gas tenemos que tener algun vce extrayendolo ya
                        	(imply (requiere_recurso_tipo ?t Gas) (recolectando_recurso_entorno Gas) )
                    	)
	                )
                ...
                (or (and (unidad_de_tipo ?unidd Segadores)	(recolectando_recurso_entorno Mineral)	(recolectando_recurso_entorno Gas)
						 (exists (?t - investigacion) (and (not (por_investigar ?t)) (investigacion_de_tipo ?t ImpulsarSegador)) )

					)
					(not (unidad_de_tipo ?unidd Segadores))
				)
    ...
    ; Accion para realizar una investigacion, necesaria para el reclutamiento de un segador
    ; edifcio -> edificio donde se realiza la investigacion
    ; inves -> investigacion que se lleva a cabo
    (:action investigar
		:parameters (?edifcio - edificio ?inves - investigacion)
		:precondition
			(and
				; para poder realizar una investigacion el edificio tiene que ser una BahiaDeIngenieria
				(edificio_de_tipo ?edifcio BahiaDeIngenieria)

				; la bahia tiene que estar ya construida en el mapa antes de usarla para la investigacion
				(not (por_construir BahiaDeIngenieria1))

				; aun no he realizado la investigacion
				(por_investigar ?inves)

				; tenemos que tener algun vce extrayendo cada recurso que necesite una investigacion antes de realizarla 
                (and 
                	(exists (?t - nombre_investigacion)  ;existe al menos un tipo de investigacion
                    	(and
                     
                        	(investigacion_de_tipo ?inves ?t) ;que sea el tipo de investigacion que vamos a realizar
                        
                        	;si ese tipo de investigacion necesita Mineral tenemos que tener algun vce extrayendolo ya
                        	(imply (requiere_recurso_tipo ?t Mineral) (recolectando_recurso_entorno Mineral) )
                    	)
	                )
                    ...
                    (exists (?t - nombre_investigacion)  ;existe al menos un tipo de investigacion
                    	(and
                        	...
                        	;si ese tipo de investigacion necesita Gas tenemos que tener algun vce extrayendolo ya
                        	(imply (requiere_recurso_tipo ?t Gas) (recolectando_recurso_entorno Gas) )
                    	)
	                )
		...
		:effect
			(and
				; indicamos que la investigacion no tiene que realizarse ya
				(not (por_investigar ?inves))
			)
	)
