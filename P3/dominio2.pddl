(define (domain ejercicio_2)
    ...
    (:action construir
      :parameters (?unidd - unidad ?edifcio - edificio ?coord - coordenadas ?recursos_entorno - recursos_entorno)
      :precondition
            (and
				...
				; necesita el recursos_entorno dado, y ese recursos_entorno se está extrayendo
				(exists (?t - nombre_edificio)
					(and
						(edificio_de_tipo ?edifcio ?t)
						(requiere_recurso_tipo ?t ?recursos_entorno)
						(recolectando_recurso_entorno ?recursos_entorno)
					)
				)
